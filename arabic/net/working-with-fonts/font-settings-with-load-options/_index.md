...
#### س: كيف يمكنني تحديد خط افتراضي عند تحميل مستند في Aspose.Words؟

 ج: لتحديد خط افتراضي عند تحميل مستند في Aspose.Words ، يمكنك استخدام`LoadOptions`فئة وضبط`DefaultFontName` الخاصية لاسم الخط المطلوب.

#### س: ما هي إعدادات الخط الأخرى التي يمكنني تحديدها باستخدام خيارات التحميل في Aspose.Words؟

ج: إلى جانب تحديد الخط الافتراضي ، يمكنك أيضًا تحديد إعدادات الخط الأخرى مثل الترميز الافتراضي باستخدام الخصائص المناسبة لـ`LoadOptions` فئة ، مثل`DefaultEncoding`.

#### س: ماذا يحدث إذا كان الخط الافتراضي المحدد غير متوفر عند تحميل المستند؟

ج: إذا لم يكن الخط الافتراضي المحدد متاحًا عند تحميل المستند في Aspose.Words ، فسيتم استخدام خط بديل لعرض النص في المستند. قد يتسبب هذا في اختلاف بسيط في المظهر من الخط الأصلي.

#### س: هل يمكنني تحديد إعدادات خط مختلفة لكل مستند تم تحميله؟

 ج: نعم ، يمكنك تحديد إعدادات خط مختلفة لكل مستند تم تحميله باستخدام مثيلات منفصلة لملف`LoadOptions` فئة وتعيين إعدادات الخط المطلوبة لكل مثيل. يتيح لك ذلك تخصيص مظهر الخط لكل مستند بشكل مستقل.