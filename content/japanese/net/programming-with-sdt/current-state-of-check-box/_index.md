...
特定のチェックボックスにアクセスするには、そのチェックボックスをドキュメントから取得する必要があります。Aspose.Words はチェックボックスを構造化ドキュメント タグとして扱います。次のコードは、ドキュメント内の最初の構造化ドキュメント タグを取得し、それがチェックボックスであるかどうかを確認します。
