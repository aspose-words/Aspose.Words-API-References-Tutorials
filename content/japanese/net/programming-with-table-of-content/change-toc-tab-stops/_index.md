...
この例では、ループを使用してドキュメント内の各段落をループします。次に、段落が目次結果 (TOC) スタイルを使用してフォーマットされているかどうかを確認します。フォーマットされている場合は、この段落で使用されている最初のタブにアクセスし、古いタブを削除して位置を変更した新しいタブを追加することでタブを変更します。
