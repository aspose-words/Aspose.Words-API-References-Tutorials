...
このチュートリアルでは、Aspose.Words for .NET の「ドキュメント テキスト方向」機能用に提供されている C# ソース コードを調べます。この機能を使用すると、ドキュメント内のテキストの方向を指定できます。これは、ヘブライ語やアラビア語など、右から左に書かれる言語に特に便利です。
...
この機能は、特定のテキスト方向を必要とする言語でワードプロセッサを使用する場合に特に便利であることに注意することが重要です。 Aspose.Words は、ドキュメント内のテキストの方向を管理する強力なツールを提供することで、このタスクを簡単にします。
