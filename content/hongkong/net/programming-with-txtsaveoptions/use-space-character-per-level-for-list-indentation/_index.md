...
在本指南中，我們說明如何使用 Aspose.Words for .NET 來套用「每層使用一個空格字元進行清單縮排」功能。透過依照提供的步驟並使用提供的 C# 原始程式碼，您可以輕鬆配置 Word 文件中清單的縮進，每層使用一個空格字元。 Aspose.Words 透過文字格式設定和清單管理為文字處理提供了巨大的靈活性和強大功能，讓您在 C# 應用程式中建立結構良好的文件。
