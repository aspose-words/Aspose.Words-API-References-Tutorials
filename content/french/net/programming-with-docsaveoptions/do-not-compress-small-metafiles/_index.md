...
La première étape consiste à définir le répertoire dans lequel vous souhaitez enregistrer le document. Vous devez spécifier le chemin complet du répertoire. Par exemple :
...
Dans cet exemple, nous créons un nouvel objet DocSaveOptions pour définir les options de sauvegarde.

## Étape 4 : Activer la fonctionnalité « Ne pas compresser les petits métafichiers »

 Pour activer la fonctionnalité "Ne pas compresser les petits métafichiers", vous devez définir le`Compliance` propriété de l'objet DocSaveOptions à la valeur`PdfCompliance.PdfA1a`. Voici comment:
...
Enfin, vous pouvez enregistrer le document en utilisant le`Save` méthode de la classe Document. Spécifiez le chemin complet du fichier et le nom de fichier souhaité. Par exemple :
