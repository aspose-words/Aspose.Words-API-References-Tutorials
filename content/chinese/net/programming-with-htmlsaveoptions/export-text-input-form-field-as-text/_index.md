...
在本教程中，我们将引导您完成 C# 源代码，使用 Aspose.Words for .NET 将文本输入表单字段导出为纯文本。此功能允许您将文本输入表单字段导出为可读文本，而不是将其导出为 HTML 输入元素。
...
现在我们将配置 HTML 保存选项，以将文本输入表单字段导出为纯文本。使用以下代码：
...
此代码创建`HtmlSaveOptions`并设置`ExportTextInputFormFieldAsText`选择`true`将文本输入表单字段导出为纯文本。此外，它还指定将保存提取的图像的文件夹。
...
此代码通过将文本输入表单字段导出为纯文本将文档转换为 HTML，并将导出的 HTML 文件保存到指定目录。
