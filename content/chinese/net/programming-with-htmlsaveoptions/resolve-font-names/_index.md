...
在本教程中，我们将引导您使用 C# 源代码使用 Aspose.Words for .NET 解决丢失的字体名称。此功能允许您在将文档转换为 HTML 时自动解析丢失的字体名称。
...
现在我们将配置 HTML 保存选项以解决转换过程中缺少字体名称的问题。使用以下代码：
...
此代码创建`HtmlSaveOptions`并设置`ResolveFontNames`选择`true`解决转换为 HTML 时缺少字体名称的问题。此外，`PrettyFormat`选项设置为`true`获得格式良好的 HTML 代码。
...
此代码通过自动解析缺失的字体名称将文档转换为 HTML，并将转换后的 HTML 文件保存到指定的目录。
