...
在本教程中，我们将引导您完成 C# 源代码，以使用 Aspose.Words for .NET 添加 CSS 类名前缀。此功能允许您在将文档转换为 HTML 时向生成的 CSS 类名添加自定义前缀。
...
此代码创建`HtmlSaveOptions`并设置`CssStyleSheetType`到`CssStyleSheetType.External`生成外部 CSS 样式表，以及`CssClassNamePrefix`到`"pfx_"`前缀`"pfx_"`命名 CSS 类。
...
### 使用 Aspose.Words for .NET 添加 Css 类名前缀的示例源代码
...
现在，您已经了解了如何在使用 Aspose.Words for .NET 将文档转换为 HTML 时添加 CSS 类名前缀。按照本教程中提供的分步指南步骤，您可以在转换后的 HTML 文档中自定义 CSS 类名。