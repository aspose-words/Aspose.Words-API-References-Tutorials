...
## 第 3 步：查找表、行和单元格索引
接下来，我们将使用 Aspose.Words for .NET 提供的方法在数组中查找表、行和单元格索引。使用以下代码：
...
这里我们使用`GetChildNodes`方法获取文档中的所有表格。然后我们使用`IndexOf`在所有表的集合中查找特定表的索引。同样，我们使用`IndexOf`查找表中最后一行的索引，并且`IndexOf`在行内查找特定单元格的索引。
