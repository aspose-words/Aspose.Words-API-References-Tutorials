...
description: 了解如何使用 Aspose.Words for .NET 附加带有导入格式选项的文档。
type: docs
weight: 10
url: /zh/net/join-and-append-documents/append-with-import-format-options/
---

本教程介绍如何使用 Aspose.Words for .NET 将一个文档的内容附加到另一个文档，并附带导入格式选项。提供的源代码演示了如何打开源文档和目标文档、指定导入格式选项以及将源文档附加到目标文档。
...
创建一个实例`ImportFormatOptions`类来指定导入格式选项。在此示例中，我们使用`KeepSourceNumbering`属性，以确保如果与目标文档发生冲突，则使用源文档的编号。
...
使用`AppendDocument`方法来追加源文档。传递`ImportFormatMode.UseDestinationStyles`作为第二个参数来使用目标文档的样式和格式。
