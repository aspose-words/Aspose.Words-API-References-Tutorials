...
为了确保源文档的内容在附加到目标文档时连续流动，您需要设置`SectionStart`源文档中第一部分的属性`SectionStart.Continuous`.
...
现在，您可以使用以下命令将源文档附加到目标文档`AppendDocument`的方法`Document`班级。这`ImportFormatMode.KeepSourceFormatting`参数确保在追加操作期间保留源格式（包括列表的格式）。
...
最后，使用启用列表保留源格式功能保存合并的文档`Save`的方法`Document`班级。
