...
```
 ...
 In diesem Schritt konfigurieren wir Backup-Optionen für Images. Wir erstellen ein neues`ImageSaveOptions`Objekt, das das gewünschte Speicherformat angibt, hier „Png“ für das PNG-Format. Wir definieren auch die Seite, die in das Bild aufgenommen werden soll, den Schwarzweiß-Farbmodus und das indizierte 1-bpp-Pixelformat.
...
```
 ...
 In diesem letzten Schritt speichern wir die Dokumentbilder im PNG-Format mit dem`Save` Methode und Übergabe des Pfads zur Ausgabedatei zusammen mit den angegebenen Speicheroptionen.

Jetzt können Sie den Quellcode ausführen, um die Dokumentbilder im PNG-Format mit einer Farbtiefe von 1 bpp indexiert zu formatieren. Die resultierende Datei wird im angegebenen Verzeichnis unter dem Namen „WorkingWithImageSaveOptions.Format1BppIndexed.Png“ gespeichert.
...
In diesem Tutorial haben wir die 1Bpp-Indexformatfunktion mit Aspose.Words für .NET untersucht. Wir haben gelernt, wie man Bilder in einem Dokument im PNG-Format mit einer Farbtiefe von 1 Bit pro Pixel (1 bpp) und einem indizierten Farbmodus formatiert.
