...
```
 ...
 Ở bước này, chúng tôi đang sử dụng`GetChild`phương thức để truy xuất phần tử OfficeMath đầu tiên từ tài liệu. Các thông số`NodeType.OfficeMath, 0, true` chỉ định rằng chúng tôi đang tìm kiếm sự xuất hiện đầu tiên của nút OfficeMath.
...
Tuyệt đối! Aspose.Words for .NET hỗ trợ chuyển đổi tài liệu Word sang nhiều định dạng khác nhau, bao gồm cả PDF. Bạn có thể sử dụng`Save` phương pháp với các định dạng khác nhau.
