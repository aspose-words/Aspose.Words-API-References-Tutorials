...
// Защита документа работает только в том случае, если защита документа отключена и разрешено только редактирование в полях формы.
doc.Protect(ProtectionType.AllowOnlyFormFields, "password");
...
### Часто задаваемые вопросы о том, как разрешить защиту только полей формы в документе Word
...
2.  Вставляйте разделы и текст в документ с помощью`DocumentBuilder` класс.
...
4.  Сохраните защищенный документ с помощью`Save` метод`Document` класс.
...
#### Вопрос: Могу ли я снять защиту с защищенного документа?
 ...
 О: Да, вы можете снять защиту с защищенного документа с помощью Aspose.Words for .NET. Чтобы снять защиту, вы можете использовать команду`Unprotect` метод`Document` class и укажите правильный пароль. Это снимет защиту и позволит неограниченное редактирование документа.

#### Вопрос: Можно ли защитить документ несколькими типами защиты?

 О: Нет, Aspose.Words for .NET позволяет одновременно применять к документу только один тип защиты. Однако`AllowOnlyFormFields` Тип защиты может эффективно ограничивать редактирование полей формы, одновременно разрешая другие типы защиты, такие как`AllowOnlyComments` или`AllowOnlyRevisions`в сочетании с защитой поля формы.
