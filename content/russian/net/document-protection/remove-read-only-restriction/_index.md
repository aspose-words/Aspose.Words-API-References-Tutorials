...
1.  Создайте экземпляр`Document` class и установите пароль для документа, используя`SetPassword` метод`WriteProtection` Объект Object.
...
4.  Сохраните документ без ограничения «только для чтения», используя команду`Save` метод`Document` Объект Object.
