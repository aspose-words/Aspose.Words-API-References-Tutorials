...
Здесь мы используем документ для получения первой таблицы, затем включаем автоматическую подгонку таблицы по`AllowAutoFit` свойство. Затем мы извлекаем первую ячейку из первой строки таблицы. Из этой ячейки мы можем получить предпочтительный тип ширины с помощью`PreferredWidth.Type` имущество и его стоимость с учетом`PreferredWidth.Value` свойство.
