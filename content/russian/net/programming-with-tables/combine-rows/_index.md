...
В этом уроке мы узнаем, как использовать Aspose.Words для .NET для объединения строк таблиц в документе Word. Мы будем следовать пошаговому руководству, чтобы понять код и реализовать эту функцию. В конце этого руководства вы сможете программно манипулировать строками таблиц в документах Word и объединять их.
...
```csharp
	...
	// с разным количеством ячеек и шириной можно объединить в одну таблицу.
	while (secondTable.HasChildNodes)
		firstTable.Rows.Add(secondTable.FirstRow);
	secondTable.Remove();
	doc.Save(dataDir + "WorkingWithTables.CombineRows.docx");
