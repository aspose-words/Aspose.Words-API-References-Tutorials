...
В этом уроке мы узнаем, как использовать Aspose.Words для .NET для объединения строк таблиц в документе Word. Мы будем следовать пошаговому руководству, чтобы понять код и реализовать эту функцию. В конце этого руководства вы сможете программно манипулировать строками таблицы и объединять их в документы Word.
...
Далее мы объединим строки второй таблицы до конца первой таблицы. Используйте следующий код:
...
```
 ...
 Здесь мы используем`while` цикл для перебора всех строк второго массива и добавления их в конец первого массива с помощью`Add` метод. Далее мы удаляем вторую таблицу из документа с помощью команды`Remove` метод.
...
```csharp
	...
	// При разном количестве ячеек и ширине их можно объединить в одну таблицу.
	while (secondTable.HasChildNodes)
		firstTable.Rows.Add(secondTable.FirstRow);
	secondTable.Remove();
	doc.Save(dataDir + "WorkingWithTables.CombineRows.docx");
...
## Заключение
В этом уроке мы узнали, как объединить строки таблиц в документе Word с помощью Aspose.Words для .NET. Следуя этому пошаговому руководству и реализовав предоставленный код C#, вы сможете программно манипулировать строками таблиц в документах Word. Эта функция позволяет эффективно объединять и организовывать данные в таблицу.