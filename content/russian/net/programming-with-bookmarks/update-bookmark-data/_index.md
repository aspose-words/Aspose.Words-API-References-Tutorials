...
Этот фрагмент кода сначала определяет путь к каталогу, в котором находится ваш документ Word. Заменять`"YOUR_DOCUMENT_DIRECTORY"` с фактическим путем в вашей системе. Затем он создает новый`Document` объект, по сути открывая указанный документ Word (`Bookmarks.docx` в этом примере).
...
```
 ...
 Эта строка возвращает закладку с именем`"MyBookmark1"` из документа. Не забудьте заменить`"MyBookmark1"` с фактическим именем закладки, на которую вы хотите настроить таргетинг в своем документе. Если закладка не существует, будет выдано исключение, поэтому убедитесь, что вы указали правильное имя.
