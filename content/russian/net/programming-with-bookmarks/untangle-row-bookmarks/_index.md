...
Для каждой закладки мы извлекаем родительскую строку как начала закладки (`bookmark.BookmarkStart`) и конец закладки (`bookmark.BookmarkEnd` ) используя`GetAncestor` метод.
...
## Шаг 4. Проверьте распутывание
 ...
 В то время`Untangle`Функция должна обеспечивать безопасность других закладок, всегда полезно проверять. Вот как мы можем проверить, не удалил ли процесс распутывания случайно конец другой закладки:
