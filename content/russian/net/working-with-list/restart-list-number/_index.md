...
#### Вопрос: Как перезапустить нумерацию списка в Aspose.Words?
 ...
 О: Чтобы перезапустить нумерацию списка в Aspose Words, вы можете использовать команду.`ListRestartAtNumber` метод`List` класс. Этот метод позволяет установить новое значение набора, с которого следует перезапустить список. Например, вы можете использовать`list.ListRestartAtNumber(1)` чтобы возобновить нумерацию с 1.
