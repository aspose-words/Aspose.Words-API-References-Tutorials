...
## Понимание функции «Игнорировать текст внутри удаленных редакций»
...
## Шаг 2. Вставка неотредактированного текста в документ.
 ...
 Когда у нас есть документ, мы можем вставить непроверенный текст, используя`DocumentBuilder` Объект Object. Например, чтобы вставить текст «Удаленный текст», мы можем использовать`Writeln` и`Write` методы:
...
## Шаг 4. Применение функции «Игнорировать текст внутри удаления редакций»
 ...
 Теперь, когда мы подготовили наш документ, удалив абзац, мы можем включить функцию «Игнорировать текст внутри удаления редакций», используя`FindReplaceOptions` Объект Object. Мы установим`IgnoreDeleted`собственность`true`:
...
Вот полный пример исходного кода, демонстрирующий использование функции «Игнорировать текст внутри удаления ревизий» в Aspose.Words для .NET:
...
#### Вопрос: Как создать новый документ в Aspose.Words для .NET?
 ...
 О: Прежде чем начать манипулировать текстом в документе, вам необходимо создать новый документ с помощью Aspose.Words for .NET. Это можно сделать, создав экземпляр`Document` Объект Object. Вот пример кода для создания нового документа:
...
#### Вопрос: Как вставить неотредактированный текст в документ с помощью Aspose.Words for .NET?
 ...
 О: Если у вас есть документ, вы можете вставить непроверенный текст, используя`DocumentBuilder` Объект Object. Например, чтобы вставить текст «Удаленный текст», вы можете использовать команду`Writeln` и`Write` методы:
...
О: Чтобы проиллюстрировать использование функции «Игнорировать текст внутри удаления редакций», мы удалим абзац из документа, используя отслеживание редакций. Это позволит нам увидеть, как эта функция влияет на последующие операции.
...
#### Вопрос: Как включить функцию «Игнорировать текст внутри удаления ревизий» в Aspose.Words для .NET?
 ...
 О: Теперь, когда мы подготовили наш документ, удалив абзац, мы можем включить функцию «Игнорировать текст внутри удаления редакций», используя`FindReplaceOptions` Объект Object. Мы установим`IgnoreDeleted`собственность`true`:
