...
#### В: Что такое функция «Игнорировать текст внутри вставки изменений» в Aspose.Words для .NET?

О: Функция «Игнорировать текст внутри ревизий вставки» в Aspose.Words для .NET позволяет указать, следует ли игнорировать текст внутри ревизий вставки во время определенных операций, таких как поиск и замена текста. Когда эта функция включена, текст внутри ревизий вставки не учитывается во время операций.
...
#### В: Как я могу вставить текст с отслеживанием изменений в Aspose.Words для .NET?

A: Когда у вас есть документ, вы можете вставить текст с отслеживанием изменений, используя`DocumentBuilder` объект. Например, чтобы вставить текст «Вставлено» с отслеживанием изменений, вы можете использовать`StartTrackRevisions`, `Writeln` , и`StopTrackRevisions` методы:
...
#### В: Как я могу вставить неотредактированный текст в Aspose.Words для .NET?

 О: В дополнение к тексту с отслеживанием редакций вы также можете вставлять неотредактированный текст с помощью кнопки`DocumentBuilder` объект. Например, чтобы вставить текст «Текст» без правок, можно использовать`Write` метод:

```csharp
builder.Write("Text");
```

#### В: Как я могу игнорировать текст внутри ревизий вставки в Aspose.Words для .NET?

 A: Чтобы игнорировать текст внутри ревизий вставки при последующих операциях, вы можете использовать`FindReplaceOptions` объект и установить`IgnoreInserted` собственность на`true`:
...
#### В: Как я могу включить ревизии вставки в результат вывода в Aspose.Words для .NET?

 A: Чтобы включить текст внутри ревизий вставки в результат вывода, вы можете изменить параметры, чтобы не игнорировать ревизии вставки. Для этого можно установить`IgnoreInserted` собственность`FindReplaceOptions` Возражать`false`:

```csharp
options.IgnoreInserted = false;
```

#### Q: Как я могу отобразить измененный документ со вставленными ревизиями в Aspose.Words для .NET?

A: После изменения параметров для включения ревизий вставки вы можете снова выполнить поиск и замену, чтобы получить результат с текстом внутри включенных ревизий вставки:
