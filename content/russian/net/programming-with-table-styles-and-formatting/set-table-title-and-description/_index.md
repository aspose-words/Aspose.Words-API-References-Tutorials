...
## Шаг 2. Загрузите документ, содержащий таблицу.
 Далее вам необходимо загрузить документ, содержащий таблицу, используя команду`Document` класс. Обязательно укажите правильный путь к документу.
...
## Шаг 3. Получите доступ к таблице и задайте заголовок и описание.
 Теперь вы можете получить доступ к таблице в документе, используя команду`GetChild()` метод и`Table` класс. Затем задайте заголовок и описание таблицы, используя`Title` и`Description` характеристики.
...
## Шаг 4. Установите параметры резервного копирования
 Если вы хотите указать параметры сохранения, вы можете настроить их с помощью`OoxmlSaveOptions` класс. В этом примере мы использовали`Compliance` Возможность указать соответствие строгому формату ISO 29500:2008.
...
## Шаг 5. Оптимизируйте совместимость документов
 Вы также можете оптимизировать совместимость документов с помощью`OptimizeFor()` метод`CompatibilityOptions` класс. В этом примере мы оптимизировали документ для Word 2016.
...
## Шаг 6. Сохраните измененный документ.
 Наконец, вы можете сохранить измененный документ в файл, используя команду`Save()` метод`Document` класс. Обязательно укажите правильный путь и имя файла.
