...
## Шаг 1. Загрузите исходные и целевые документы:
 ...
 Определить строковую переменную`dataDir` чтобы сохранить путь к каталогу ваших документов. Замените «ВАШ СПРАВОЧНИК ДОКУМЕНТОВ» на фактическое местоположение.
...
## Шаг 3. Включение перезапуска нумерации страниц:
 ...
 В пределах того же самого`PageSetup` объекта первого раздела исходного документа, установите`RestartPageNumbering`собственность`true`. Этот важный шаг указывает Aspose.Words заново начать нумерацию страниц для добавленного контента.
