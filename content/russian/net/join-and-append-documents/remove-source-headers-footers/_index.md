...
description: Узнайте, как удалять верхние и нижние колонтитулы при объединении и добавлении документов Word с помощью Aspose.Words для .NET.
type: docs
weight: 10
...
## Шаг 3. Удаление верхних и нижних колонтитулов из разделов исходного документа
 ...
 Чтобы удалить верхние и нижние колонтитулы из каждого раздела исходного документа, вы можете перебирать разделы, используя`foreach` зациклиться и вызвать`ClearHeadersFooters` метод.
...
## Шаг 6: Сохраните окончательный документ
 ...
 Наконец, сохраните объединенный документ с включенной функцией «Удалить исходные колонтитулы», используя кнопку`Save` метод`Document` класс.
...
```csharp
	...
	// Даже после того, как верхние и нижние колонтитулы будут удалены из исходного документа, параметр «LinkToPrevious»
