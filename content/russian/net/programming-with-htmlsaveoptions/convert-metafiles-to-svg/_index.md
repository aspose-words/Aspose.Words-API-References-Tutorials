...
```
 ...
 Этот код создает экземпляр`Document`и`DocumentBuilder` для построения документа. Он вставляет`<svg>` тег, содержащий`<polygon>` элемент с атрибутами, определяющими форму и стиль изображения SVG.
