...
В этом коде мы создаем новый документ и добавляем в него текст, в том числе текст на разных языках.

## Шаг 2. Определите параметры сохранения текста

Далее нам нужно определить параметры сохранения текста, которые определяют, как документ должен быть сохранен в виде текстового файла. Мы можем настроить различные параметры, такие как добавление меток биди, отступ списка и многое другое. Давайте рассмотрим два примера:
...
```
 ...
 В этом примере мы создаем`TxtSaveOptions` объект и установите`AddBidiMarks`собственность`true` для включения меток биди в текстовый вывод.
...
Здесь мы настраиваем параметры сохранения для использования символа табуляции для отступа списка со счетом 1.
