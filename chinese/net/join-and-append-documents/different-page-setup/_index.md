...
本教程解释了如何使用 Aspose.Words for .NET 将具有不同页面设置设置的文档附加到另一个文档。提供的源代码演示了如何为源文档和目标文档设置不同的页面设置，并确保正确的连续性和编号。
...
调整源文档的页面设置以确保正确的连续性和编号。在此示例中，我们将部分开始设置为`SectionStart.Continuous`并重新开始页码。我们还确保页面宽度、高度和方向与目标文档的最后一部分匹配。
...
为了保持正确的格式，遍历源文档中的所有段落并设置`KeepWithNext`财产给`true`.这可确保段落在附加过程中保持在一起。
...
使用`AppendDocument`目标文档的方法将修改后的源文档附加到目标文档，同时保留源格式。
...
### 使用 Aspose.Words for .NET 的不同页面设置示例源代码 
