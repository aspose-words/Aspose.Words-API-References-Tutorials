...
#### Как обращаться с зашифрованными файлами документов во время определения формата?
 ...
 При определении формата файла документа вы можете использовать`IsEncrypted` собственность`FileFormatInfo` объект, чтобы проверить, зашифрован ли файл. Если файл зашифрован, вы можете предпринять дополнительные шаги для обработки этого конкретного случая, например, скопировать файл в каталог, предназначенный для зашифрованных документов. Вы можете использовать`File.Copy` способ сделать это.
