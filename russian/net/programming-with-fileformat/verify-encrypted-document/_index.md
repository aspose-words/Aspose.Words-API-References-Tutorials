...
### В: Каковы шаги для проверки зашифрованного документа Word?
...
### Q: Как я могу установить каталог документов?
 Чтобы установить каталог документов, вам нужно заменить`"YOUR DOCUMENT DIRECTORY"` с фактическим путем к каталогу ваших документов в следующем коде:

```csharp
string dataDir = "YOUR DOCUMENT DIRECTORY";
```

### В: Как определить формат файла?
 Вы можете использовать`DetectFileFormat` метод`FileFormatUtil` класс для обнаружения информации о формате файла. В следующем примере предполагается, что зашифрованный документ называется «Encrypted.docx» и находится в указанном каталоге документов:
...
### В: Как проверить, зашифрован ли документ с помощью Aspose.Words для .NET?
Выполнив шаги, указанные в этом руководстве, и запустив предоставленный исходный код, вы сможете проверить, зашифрован ли документ с помощью Aspose.Words для .NET.
