...
#### В: Что такое метасимволы в шаблоне поиска в Aspose.Words для .NET?

О: Функция «Метасимволы в шаблоне поиска» в Aspose.Words для .NET позволяет использовать специальные метасимволы для расширенного поиска и замены в документах Word. Эти метасимволы позволяют представлять разрывы абзацев, разделов, страниц и другие специальные элементы в шаблоне поиска.

#### В: Как создать новый документ в Aspose.Words для .NET?

 О: Прежде чем использовать метасимволы в шаблоне поиска, вы должны создать новый документ, используя Aspose.Words для .NET. Это можно сделать, создав экземпляр`Document` объект. Вот пример кода для создания нового документа:
...
#### В: Как вставить текст в документ с помощью Aspose.Words для .NET?
 ...
 A: Когда у вас есть документ, вы можете вставить текст, используя`DocumentBuilder` объект. В нашем примере мы используем`Writeln` и`Write` методы для вставки двух строк текста:
...
#### В: Как искать и заменять текст метасимволами в документе с помощью Aspose.Words для .NET?

 A: Для поиска и замены текста метасимволами вы можете использовать`Range.Replace` метод. В нашем примере мы заменяем фразу «Это строка 1&pЭто строка 2» на «Эта строка заменена», используя`&p` метасимвол для представления разрыва абзаца:

```csharp
doc.Range.Replace("This is row 1&pThis is row 2", "This row is replaced");
```

#### В: Как вставить разрыв страницы в документ с помощью Aspose.Words для .NET?

A: Чтобы проиллюстрировать использование другого метасимвола, мы вставим разрыв страницы в документ, используя`InsertBreak` метод с`BreakType.PageBreak` параметр. Сначала мы перемещаем курсор из`DocumentBuilder` в конец документа, затем вставляем разрыв страницы и новую строку текста:
...
#### В: Как найти и заменить другим метасимволом в документе с помощью Aspose.Words для .NET?

 A: Теперь мы выполним еще один поиск и замену, используя`&m` метасимвол для представления разрыва страницы. Мы заменяем фразу «Это строка 1&mЭто строка 2» на «Разрыв страницы заменяется новым текстом». :
